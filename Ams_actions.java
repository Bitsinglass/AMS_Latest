...
//	  Add_asset_Submit_btn.click();
  ...
  public void Submit_frm() throws InterruptedException
  {
 ...
 {
	System.out.println("index_ele " +index_ele); 
	List_assets.get(index_ele).findElements(By.xpath("//i[@data-test-id='202203300135520890667']")).get(index_ele-1).click();
 }
 
 public int Asset_page_count() 
 {
     WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(10));
     wait.until(ExpectedConditions.elementToBeClickable(Assets_pg_count));
	 int rec= Integer.parseInt(Assets_pg_count.getText());
	 return rec;
 }
